...
def three_min_spanning_trees(input_file_path, output_file_path):
    ...
    excluded_edges2 = set()
    if mst_edges1:
        largest_edge = max(mst_edges1, key=lambda x: x[2])
    ...
    if mst_edges1 and mst_weight2 != float('inf'):
        second_largest_edge = max([edge for edge in mst_edges1 if (edge[0], edge[1]) not in excluded_edges2], key=lambda x: x[2])
...
three_min_spanning_trees("test_cases/input1.txt", "test_cases/output.txt")